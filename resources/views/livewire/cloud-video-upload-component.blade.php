        ...
        @if(count($videos) === 0)
            <div class="space-y-4">
                ...
                <p class="text-xs text-love-500 font-mono">
                    Max 1GB per file • MP4, WebM, MOV, AVI, MKV
                </p>
            </div>
        @else
            <div class="space-y-2">
                <p class="text-lg font-semibold text-love-900">
                    {{ count($videos) }} video(s) selected
                </p>
                @if($storageCheck && $storageCheck['can_upload'])
                    <p class="text-sm text-love-600">
    ...
    @if(count($videos) > 0)
        <div class="mt-6 space-y-3">
            @foreach($videos as $index => $video)
                <div class="bg-love-50 rounded-xl overflow-hidden border border-love-200 p-4">
                    <div class="flex items-start gap-4">
                        <div class="w-16 h-16 bg-love-200 rounded-lg flex items-center justify-center flex-shrink-0">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-love-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                            </svg>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-love-900 truncate font-mono">{{ $video->getClientOriginalName() }}</p>
                            <p class="text-xs text-love-600 font-mono">{{ formatBytes($video->getSize()) }}</p>
                            <input
                                type="text"
                                wire:model="captions.{{ $index }}"
                                placeholder="Add caption (optional)"
                                class="mt-2 w-full px-3 py-2 border border-love-200 rounded-lg text-sm text-love-900 focus:outline-none focus:border-love-500"
                                :disabled="$isUploading"
                            />
                        </div>
                        <button
                            wire:click="removeVideo({{ $index }})"
                            class="w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center flex-shrink-0"
                            :disabled="$isUploading"
                        >
        ...
        <div class="mt-6 flex gap-3">
            ...
            <button
                wire:click="upload"
                type="button"
                class="flex-1 py-3 bg-gradient-to-r from-love-700 to-love-800 hover:from-love-800 hover:to-love-900 text-white rounded-xl font-semibold transition-all duration-200 flex items-center justify-center gap-2"
                :disabled="$isUploading"
            >
                @if($isUploading)
        ...
        @if($isUploading)
            <div class="mt-4">
                <div class="h-2 bg-love-100 rounded-full overflow-hidden">
                    <div class="h-full bg-gradient-to-r from-love-700 to-love-800 transition-all duration-300" style="width: {{ $uploadProgress }}%"></div>
                </div>
